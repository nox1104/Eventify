...
CONVERSION_FLAG_FILE = "utc_conversion_done.flag"

# Funktion zur einmaligen Konvertierung aller Events nach UTC (für Sommerzeit)
def convert_existing_events_to_utc():
    """
    Einmalige Konvertierung aller bestehenden Events nach UTC.
    Geht davon aus, dass alle Events in der Sommerzeit (UTC+2) erstellt wurden.
    """
    # Prüfen, ob die Konvertierung bereits durchgeführt wurde
    if os.path.exists(CONVERSION_FLAG_FILE):
        logger.info("UTC-Konvertierung wurde bereits durchgeführt, überspringe...")
        return
    
    logger.info("Starte einmalige Konvertierung aller Events nach UTC...")
    ...
    try:
        ...
        if os.path.exists(EVENTS_JSON_FILE):
            with open(EVENTS_JSON_FILE, 'r', encoding='utf-8') as source:
                with open(backup_path, 'w', encoding='utf-8') as backup:
                    backup.write(source.read())
            logger.info(f"Backup erstellt: {backup_path}")
        else:
            logger.info(f"Keine bestehende events.json gefunden, überspringe Backup")
            # Erstelle leere events.json falls sie nicht existiert
            with open(EVENTS_JSON_FILE, 'w', encoding='utf-8') as f:
                json.dump({"events": []}, f, indent=4)
            # Erstelle Flag-Datei und beende
            with open(CONVERSION_FLAG_FILE, 'w') as f:
                f.write(f"UTC conversion not needed. Flag created on {datetime.now().isoformat()}")
            return
    ...
    try:
        ...
        if not isinstance(events_data, dict) or "events" not in events_data:
            logger.error("Ungültiges Format in events.json. Überspringe Konvertierung.")
            return
        ...
        for event in events_data["events"]:
            ...
            # Zeitobjekt als String verarbeiten
            if "datetime_obj" in event and event["datetime_obj"]:
                dt_str = event["datetime_obj"]
                
                # Prüfen, ob bereits eine Zeitzone angegeben ist
                if "+" in dt_str or "Z" in dt_str:
                    logger.info(f"Überspringe {title}: Bereits mit Zeitzone")
                    continue
                    
                # Zeit extrahieren
                time_match = re.search(r'T(\d{2}):(\d{2})', dt_str)
                if time_match:
                    hour = int(time_match.group(1))
                    minute = int(time_match.group(2))
                    
                    # Sommerzeitverschiebung (-2 Stunden für UTC+2)
                    offset = 2
                    
                    # Neue Uhrzeit berechnen
                    new_hour = hour - offset
                    if new_hour < 0:
                        # Wenn die Stunde negativ wird, gehe einen Tag zurück
                        try:
                            date_part = dt_str.split('T')[0]
                            date_obj = datetime.fromisoformat(date_part)
                            prev_day = date_obj - timedelta(days=1)
                            date_part = prev_day.isoformat().split('T')[0]
                            new_hour += 24
                            
                            # Datum und Zeit neu zusammensetzen
                            new_dt_str = f"{date_part}T{new_hour:02d}:{minute:02d}:00+00:00"
                        except Exception as e:
                            logger.error(f"Fehler beim Datumswechsel für {title}: {e}")
                            continue
                    else:
                        # Einfacher Fall: Nur die Stunde anpassen
                        new_dt_str = re.sub(r'T\d{2}:', f'T{new_hour:02d}:', dt_str) + "+00:00"
                    
                    old_dt_str = dt_str
                    event["datetime_obj"] = new_dt_str
                    converted += 1
                    logger.info(f"Konvertiert: {title}: {old_dt_str} -> {new_dt_str}")
                ...
                if len(parts) == 2 and len(parts[0]) == 12 and parts[0].isdigit():
                    try:
                        ...
                        if new_timestamp != parts[0]:
                            ...
                            updated_ids += 1
                            logger.info(f"ID aktualisiert: {title}: {old_id} -> {event['event_id']}")
                    except Exception as e:
                        logger.error(f"ID-Fehler bei {title}: {e}")
        
        # Speichern
        if converted > 0 or updated_ids > 0:
            logger.info(f"{converted} Events konvertiert und {updated_ids} IDs aktualisiert.")
            with open(EVENTS_JSON_FILE, "w", encoding="utf-8") as f:
                json.dump(events_data, f, indent=4)
        else:
            logger.info("Keine Änderungen notwendig.")
        
        # Flagdatei erstellen
        with open(CONVERSION_FLAG_FILE, 'w') as f:
            f.write(f"UTC conversion completed on {datetime.now().isoformat()}. Converted {converted} events.")
        
        logger.info("UTC-Konvertierung abgeschlossen. Flagdatei erstellt.")
        
    except Exception as e:
        logger.error(f"Fehler bei der Konvertierung: {e}")

# Führe die Konvertierung beim Start aus
convert_existing_events_to_utc()
